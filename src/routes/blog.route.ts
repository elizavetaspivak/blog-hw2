...
blogRoute.get('/', (req: RequestType, res: ResponseType<OutputBlogType[]>) => {
    const bloggers = BlogsRepository.getAllBlogs()

    res.status(HTTP_RESPONSE_CODES.SUCCESS).send(bloggers)
})

blogRoute.get('/:id', (req: RequestWithParams<ParamType>, res: ResponseType<OutputBlogType>) => {
...
blogRoute.post('/', authMiddleware, blogValidation(), (req: RequestWithBody<CreateBlogModel>, res: ResponseType<OutputBlogType>) => {
...
blogRoute.put('/:id', authMiddleware, blogValidation(), (req: RequestWithParamsAndBody<ParamType, UpdateBlogModel>, res: ResponseType<void>) => {
    const id = req.params.id
...
blogRoute.delete('/:id', authMiddleware, (req: RequestWithParams<ParamType>, res: ResponseType<void>) => {
    const id = req.params.id
